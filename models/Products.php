...
class Products extends Model {
	...
	public function getAvailableValuesFromOptions($groups, $ids){
		...
		$sql = "SELECT p_value, id_option, COUNT(id_option) as c FROM products_options WHERE id_option IN ('".implode("','", $groups)."') AND id_product IN ('".implode("','", $groups)."') GROUP BY p_value ORDER BY id_option";
	...
	public function getProductInfo($id){
		$array = array();
		if(!empty($id)){
			$sql = "SELECT *, (select brands.name from brands where brands.id = products.id_brand) as brand_name FROM products WHERE id = :id";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(":id", $id);
			$sql->execute();
			if ($sql->rowCount() > 0) {
	...
	public function getOptionsByProductId($id){
		...
		if($sql->rowCount() > 0){
			...
			//Etapa 3 - Junta tudo em um único array
